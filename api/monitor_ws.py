...
async def monitor_websocket(websocket: WebSocket) -> None:
    ...
    # 控制台提示连接状态
    print(f'[OK] Monitor client connected: {client_id}')

    # 发布前端连接事件，便于后端感知监听器变化
    EventBus.publish(MonitorEventType.FRONTEND_CONNECTED, {'client_id': client_id})

    # 发送历史事件，方便前端初始化状态
    history = EventBus.get_recent_events(limit=50)
...
def _subscribe_to_events() -> None:
    for event_type in MonitorEventType:
        EventBus.subscribe(event_type, lambda event: asyncio.create_task(broadcast_event_to_monitors(event)))
